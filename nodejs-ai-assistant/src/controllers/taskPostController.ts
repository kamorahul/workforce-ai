...
// Search for task by properties (optimized for system messages)
router.get('/search', async (req: Request, res: Response) => {
  try {
    ...
    // Match by channelId (handle both formats: "messaging:channel-id" and "channel-id")
    if (channelId) {
      const channelIdStr = channelId as string;
      // Try both full format and extracted ID
      if (channelIdStr.includes(':')) {
        const extractedId = channelIdStr.split(':')[1];
        // Use $in to match either format
        query.channelId = { $in: [channelIdStr, extractedId] };
      } else {
        query.channelId = channelIdStr;
      }
    }
    ...
    // Match by createdAt (within 5 minute tolerance)
    if (createdAt) {
      try {
        const searchDate = new Date(createdAt as string);
        const tolerance = 5 * 60 * 1000; // 5 minutes in milliseconds
        query.createdAt = {
          $gte: new Date(searchDate.getTime() - tolerance),
          $lte: new Date(searchDate.getTime() + tolerance)
        };
      } catch (error) {
        console.error('Invalid createdAt format:', error);
        // Skip createdAt matching if invalid format
      }
    }
    
    // Exclude subtasks (only search top-level tasks)
    query.parentTaskId = { $exists: false };
    
    // Find matching tasks (limit to 1 for efficiency)
    const tasks = await Task.find(query)
      .limit(1)
    ...
    res.status(500).json({ 
      status: 'error',
      error: 'Failed to search for task',
      task: null
    });
  }
});
