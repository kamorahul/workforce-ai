  ...
  switch (message.command) {
    ...
    case 'calender':
      ...
      if (summaryChannel) {
        ...
        agent.handleMessage(
            `Generate all upcoming events(meetings, group call, work scheduling, timings, team events, company events, occasions etc.) details based on recent messages(if considering the current time ${new Date()}) with all the channel members for ${user.id} .`,
        );
      } else {
